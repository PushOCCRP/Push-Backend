...
class ArticlesController < ApplicationController
  ...
  def search_occrp_joomla
    ...
    response["items"].each do |result|
      ...
      url << "," if result != response["items"].last
    end
  ...
  private
    ...
    def register_consumer_event
      ...
      return unless params.key? "installation_uuid"
    ...
    def check_api_key
      ...
      auth = true
      auth = false unless ENV.key?("auth_enabled") && ENV["auth_enabled"] == "true"

      return if auth == false

      # if there's nothing passed in, kill it with fire
      auth = false unless params.key?("api_key") && params["api_key"].empty? == false
      if auth == false
    ...
    def extract_images(article)
      ...
      images_array.each do |image|
        ...
        if !image_address.starts_with?("http")
          # build up missing parts
          prefix = ""
          prefix = if image.attributes["src"].value.starts_with?(":")
            "https"
          ...
          image.remove if image == images_array.first
