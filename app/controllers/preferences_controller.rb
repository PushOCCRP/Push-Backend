...
class PreferencesController < ApplicationController
  ...
  def index
    ...
    if Setting.categories.nil? == false
      @selected_categories = YAML.safe_load(Setting.categories)
      @selected_categories = {} if @selected_categories.class == false.class
    end

    CMS.languages.each { |language| @selected_categories[language] = [] if @selected_categories.key?(language) && @selected_categories[language].blank? }

    @category_names = if !Setting.category_names.nil?
  ...
  def update
    ...
    params[:category].keys.each do |language|
      params[:category][language].delete_if(&:blank?)
    end
