...
class NotificationsController < ApplicationController
  ...
  before_action :authenticate_user!, except: %w[subscribe unsubscribe]

  skip_before_action :verify_authenticity_token, only: %w[subscribe unsubscribe]
  def index
  ...
  def subscribe
    ...
    when "android"
      ...
      dev_token = params["regid"] if params.key? "regid"
      dev_token = params["reg_id"] if params.key? "reg_id"
    when "ios"
    ...
    device = PushDevice.find_by_dev_id(params["dev_id"]) unless params["dev_id"].nil?
    if !device
  ...
  def push
    ...
    if ios_status == "SUCCESS" && android_status == "SUCCESS"
      ...
      error = ""
      error += "iOS Error: #{ios_status} " if ios_status != "SUCCESS"

      error += "Android Error: #{android_status}" if android_status != "SUCCESS"
  ...
  def admin
    ...
    @ios_devices = [] if @ios_devices.nil?

    @android_devices = [] if @android_devices.nil?
  ...
  private
    ...
    def check_push_status
      ...
      return statuses unless response_json.key?("services")

      services = response_json["services"]

      # check ios
      if services.key?("push_app-ios") && !services["push_app-ios"].empty?
        statuses[:apns_prod] = check_push_ios_status(services["push_app-ios"].first)
      end

      if services.key?("push_app-ios-sandbox") && !services["push_app-ios-sandbox"].empty?
        statuses[:apns_sandbox] = check_push_ios_status(services["push_app-ios-sandbox"].first)
      end

      # check android
      if services.key?("push_app-gcm") && !services["push_app-gcm"].empty?
    ...
    def check_push_ios_status(service)
      check_push_response_status service, %w[addr cert key service]
    end

    def check_push_android_status(service)
      check_push_response_status service, %w[apikey projectid service]
    end
    ...
    def check_push_response_status(service, required_keys)
      ...
      required_keys.each { |key| return false unless service.key?(key) }
      true
    end
end
