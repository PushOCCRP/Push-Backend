...
private
  ...
  def self.get_articles(version = 1)
    ...
    cached_articles = Rails.cache.fetch("articles_#{categories.join('_')}", expires_in: 5.minutes) do
      ...
      articles.each do |key, category|
        ...
        formatted_categories.keys.each do |category_key|
          formatted_categories[key] = rearrange_articles_for_images(formatted_categories[category_key])
        end
  ...
  # TODO: This should really be a single function taking a block, and save a bunch of code
  def self.get_categories
    ...
    logger.debug("get_categories #{param} Cache missed") if cache == false
  ...
  def self.make_request(url)
    ...
    response = HTTParty.get(CGI.encode(url), basic_auth: auth_credentials)

    begin
      body = JSON.parse response.body
    rescue
      logger.debug "Exception parsing JSON from CMS"
      logger.debug "#{excdption}"
      logger.debug ""
