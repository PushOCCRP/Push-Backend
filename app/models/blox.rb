...
class Blox < CMS
  ...
  def self.authenticate(user_name, password, _params)
  ...
  private
    ...
    def self.get_articles(_version = 1)
      ...
      YAML.safe_load(Setting.category_names)
      if !categories_string.blank? && Setting.consolidated_categories.blank?
        categories = YAML.safe_load(categories_string)["en"]
      ...
      category_names = YAML.safe_load(Setting.category_names)
      ...
      categories = category_names["en"].reject(&:empty?) unless category_names["en"].empty?
    ...
    def self.get_categories
      ...
      logger.debug("get_categories #{params} Cache hit") if cache == true
      logger.debug("get_categories #{params} Cache missed") if cache == false
    ...
    def self.get_articles_for_category(category)
      ...
      logger.debug("get_categories #{category} Cache hit") if cache == true
      logger.debug("get_categories #{category} Cache missed") if cache == false

      cached_articles
    end

    def self.parse_page(json)
      return unless json.key?("json_url")
    ...
    def self.get_article_asset(page_asset)
      ...
      cached_asset = Rails.cache.fetch("article/#{page_asset['id']}", expires_in: 1.hour) do
        ...
        article["images"] = asset["relationships"]["child"].select do |image_asset|
                              image_asset["asset_type"] == "image"
                            end.map do |image_asset|
    ...
    def self.get_asset(page_asset)
      ...
      Rails.cache.fetch("editorial/get/#{id}", expires_in: 1.hour) do
