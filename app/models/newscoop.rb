...
class NewscoopSingleton
  ...
  def get_new_access_token
    ...
    return @access_token unless check_for_expiration
...
class Newscoop < CMS
  def self.articles(params)
    ...
    if !categories.nil?
      ...
      @response = Rails.cache.fetch("categories_string/#{language}", expires_in: 1.hour) do
        ...
        begin
          categories_to_include = YAML.safe_load(Setting.categories)
        ...
        unless Setting.show_most_recent_articles.nil?
  ...
  def self.categories(full_objects = false, languages = nil)
    ...
    @response = Rails.cache.fetch("sections/#{languages.join(',')}", expires_in: 1.hour) do
      ...
      done = true unless to_return.nil?
  ...
  private
    ...
    def self.format_newscoop_articles(articles)
      ...
      articles.each do |article|
        ...
        unless article["fields"]["youtube_shortcode"].blank?
      ...
      cleaned_up_articles = formatted_articles.map do |article|
